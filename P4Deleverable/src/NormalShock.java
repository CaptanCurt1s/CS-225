...
public class NormalShock{
	...
	private double SHOCKT02toT01;	
	private double SHOCKP02toP01;
	private double SHOCKRho02toRho01;

	private double SHOCKT2toT1;
	private double SHOCKP2toP1;
	private double SHOCKRho2toRho1;	
	...
	public void NormalShockRelations(double MachIN, double gamma) {
		...
		double[] ISENTROPICP0toP;
		double[] ISENTROPICT0toT;
		double[] ISENTROPICRho0toRho;

		double StaticPressureRatio;
		double StaticTempRatio;
		double StaticDensityRatio;

		double StagnationPressureRatio;
		double StagnationTempRatio;
		double StagnationDensityRatio;

		MachHERE = new double[2];
		ISENTROPICP0toP = new double[2];
		ISENTROPICT0toT = new double[2];
		ISENTROPICRho0toRho = new double[2];
		...
		Isen.IsentropicRelations(MachHERE[0], gamma);
		ISENTROPICP0toP[0] = Isen.getISENTROPICP0toP();
		ISENTROPICT0toT[0] = Isen.getISENTROPICT0toT();
		ISENTROPICRho0toRho[0] = Isen.getISENTROPICRho0toRho();

		Isen.IsentropicRelations(MachHERE[1], gamma);
		ISENTROPICP0toP[1] = Isen.getISENTROPICP0toP();
		ISENTROPICT0toT[1] = Isen.getISENTROPICT0toT();
		ISENTROPICRho0toRho[1] = Isen.getISENTROPICRho0toRho();
		...
		StagnationPressureRatio = StaticPressureRatio 																 	// P2/P1
				* Math.pow(ISENTROPICP0toP[0]	,-1) 	// P1/P01	
				* ISENTROPICP0toP[1];				 	// P02/P2

		StagnationTempRatio = StaticTempRatio 			// T2/T1
				* Math.pow(ISENTROPICT0toT[0],-1) 		// T1/T01
				* ISENTROPICT0toT[1];			   		// T02/T2

		StagnationDensityRatio = StaticDensityRatio 	 // Rho2/Rho1
				* Math.pow(ISENTROPICRho0toRho[0], -1) 	 // Rho1/Rho01
				* ISENTROPICRho0toRho[1];				 // Rho2/Rho02
